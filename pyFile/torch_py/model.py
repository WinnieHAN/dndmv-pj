...
class AttnLSTM(nn.Module):
    def __init__(self, head_dic_size, head_dim, head_tag_lstm_size, head_word_lstm_size, head_lstm_dim, valency_size, valency_dim, direct_size,
                 direct_dim, nhid, nclass, lstm_hidden_dim, dropout_p, max_length, softmax_layer_dim):
        super(AttnLSTM, self).__init__()
        print(torch.__version__)
        self.model_type = 2  #  0 simple nn  1 lstm with atten   2 lstm  3 bag of words 4 anchored words 5 variational
        ...
        self.head_tag_lstm_embeddings = nn.Embedding(head_tag_lstm_size, head_lstm_dim)
        self.head_word_lstm_embeddings = nn.Embedding(head_word_lstm_size, head_lstm_dim)
        ...
        if self.model_type==2:
            self.combine_lstmh_hid = self.nhid + self.lstm_layer*self.lstm_direct*self.lstm_hidden_dim*2
        ...
        #################
            ...
            self.model2_linear1 = nn.Linear(self.combine_lstmh_hid , 15)
            self.model2_linear2 = nn.Linear(15, self.nclass)
            self.variational_mu = nn.Linear(self.lstm_hidden_dim*self.lstm_direct*self.lstm_layer, 10)
            self.variational_logvar = nn.Linear(self.lstm_hidden_dim*self.lstm_direct*self.lstm_layer, 10)  # log var.pow(2)
    ...
    def forward(self, sentences, sentences_w, sentences_len, h, direction_left, direction_right, v):
        ...
        if self.model_type==1:
            ...
            self.hidden = self.init_hidden(batch_size)  # sts batch
            embeds = self.head_tag_lstm_embeddings(autograd.Variable(torch.LongTensor(sentences)))
            sts_packed = torch.nn.utils.rnn.PackedSequence(embeds, batch_sizes=sentences_len)
            sentence_in = pad_packed_sequence(sts_packed, batch_first=True)
            ...
            self.w_hidden = self.init_hidden(batch_size)  # sts batch
            w_embeds = self.head_word_lstm_embeddings(autograd.Variable(torch.LongTensor(sentences_w)))
            w_sts_packed = torch.nn.utils.rnn.PackedSequence(w_embeds, batch_sizes=sentences_len)
            w_sentence_in = pad_packed_sequence(w_sts_packed, batch_first=True)
            w_lstm_out, self.w_hidden = self.lstm(w_sentence_in[0], self.w_hidden)  # [0]# sentence_in.view(BATCH_SIZE, BATCH_SIZE, -1)
            w_sentences_all_lstm = torch.transpose(self.w_hidden[0], 0, 1)
            w_sentences_all_lstm = w_sentences_all_lstm.contiguous().view(w_sentences_all_lstm.size()[0], -1)
            return torch.cat((hid_tensor, self.dropout(sentences_all_lstm), self.dropout(w_sentences_all_lstm)), 1)
        elif self.model_type==3:
            self.hidden = self.init_hidden(batch_size)  # sts batch
            embeds = self.head_lstm_embeddings(autograd.Variable(torch.LongTensor(sentences)))
            ...
            self.hidden = self.init_hidden(batch_size)  # sts batch
            embeds = self.dropout(self.head_lstm_embeddings(autograd.Variable(torch.LongTensor(sentences))))
            ...
            var_out = reparameterize(self.training, mu, logvar)
            return torch.cat((hid_tensor, var_out), 1)#, -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())


    def forwardChd(self, sentences, w_sentences, sentences_len, h, direction_left, direction_right, v):
        hids = self.forward(sentences, w_sentences, sentences_len, h, direction_left, direction_right, v)
        hid = hids[0] if isinstance(hids, tuple) else hids
    ...
    def forward_chd_train(self, sentences, w_sentences, sentences_len, h, direction_left, direction_right, v):
        hids = self.forward(sentences, w_sentences, sentences_len, h, direction_left, direction_right, v)
        hid = hids[0] if isinstance(hids, tuple) else hids
        loss = hids[1] if isinstance(hids, tuple) else 0
        if self.model_type==0:
            ...
            return (self.model2_linear2(F.relu(self.model2_linear1(hid))), loss)
