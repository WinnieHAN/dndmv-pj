...
/**
 * Softmax-EM with the sigma value changing with the sentence lengths (sigma =
 * sigma_0 / len) and iterations.
 */
public class AnnealingDynamicSoftmaxEM extends EM {
	...
	public AnnealingDynamicSoftmaxEM(AbstractModel model, double sigma_0,
			double sigma_k) {
		super(model);
	...
	public void em(int numIters, TrainStats stats)
		...
		for (iter = 0; iter < numIters; iter++) {
			...
			sigma = Math.max(0, sigma_0 + iter * sigma_k);
	...
	public void corpusEStep(DepProbMatrix counts,
		...
		for (AbstractSentenceDist asd : sentenceDists) {
			...
			double s = sigma / numWords;
			if (s > 0.95) // 0.95 instead of 1 to avoid overflow
				sentenceEStepV(sd, countsV, stats);
			else {
				sentenceEStep(sd, counts, stats, 1 / (1 - s));
			}
		}
