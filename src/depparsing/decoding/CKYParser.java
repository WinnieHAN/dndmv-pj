...
public class CKYParser {
	static HashMap<Pair<String, String>,Pair<Integer, Integer>> universalRule;
	static double regulationValue;
	public CKYParser(){
...
//		regulationValue = 0;
	 ...
	 */
	public static double parseSentence(DepSentenceDist sd, int[] parse, HashMap<Pair<String, String>,Pair<Integer, Integer>> universalRule, double regulationValue) {
	    //universalRule = universalRule;
	    //regulationValue = regulationValue;
		try { return parseSentence(sd, parse, null, null, universalRule, regulationValue); } catch(IOException e) { /* Will never happen */ };
	...
	public static double parseSentence(DepSentenceDist sd, int[] parse,
			CountAlphabet<String> tagAlphabet, String scoresFile, HashMap<Pair<String, String>,Pair<Integer, Integer>> universalRule, double regulationValue) throws IOException {
		...
		double scores[][][][] = new double[numWords + 1][numWords + 1][sd.nontermMap.numNontermTypes][numWords];//?+1 ?4* d c //@Jy numNontermTypes: 4 * max_valancy
		...
		for(int i = 0; i < numWords; i++) {
			...
			int baseIndexR = sd.nontermMap.getNontermIndex(RIGHT, CHOICE);//where is STOP/Conti?
			for(int v = 0; v < sd.nontermMap.maxValency; v++) {
				int dv = Math.min(v, sd.nontermMap.decisionValency - 1);
		...
		for(int span = 2; span <= numWords; span++) {
			for(int begin = 0; begin <= numWords - span; begin++) {
				...
				for(int split = begin + 1; split <= end - 1; split++) {
					...
					addChildProbs(begin, end, split, sd, back, scores,  universalRule,regulationValue);
				}
			...
			if(backLhs != null) {
				...
				if(isReg){
					...
					if(universalRule.containsValue(rule)){
						prob = sd.root[i] + scores[0][numWords][startValency][i] + regulationValue;
					}else{
						prob = sd.root[i] + scores[0][numWords][startValency][i];
					}
				}
	...
	private static void addChildProbs(int begin, int end, int split, DepSentenceDist sd,
							   RuleRhs[][][][] back, double[][][][] score, HashMap<Pair<String, String>,Pair<Integer, Integer>> universalRule, double regulationValue) {
				...
				for(int v = 0; v < sd.nontermMap.maxValency; v++) {
					...
					for(int dir = 0; dir < 2; dir++) {
						...
						if(leftBack != null && rightBack != null) {
							//
							boolean isReg = (regulationValue > 1e-20) ? true: false;
							double prob = 0;
							if(isReg){
								int p = sd.depInst.words[parentIndex];
								int c = sd.depInst.words[childIndex];
								Pair<Integer, Integer> rule = new Pair<Integer, Integer>(Integer.valueOf(p) ,Integer.valueOf(c));
								if(universalRule.containsValue(rule)){
									prob = sd.child[childIndex][parentIndex][cv] +//should correct here!
...
//			System.out.println(begin + ' ' + end + ' ' + rhsU.B + ' ' + word);
	 ...
	 */
	public static double[] computeAccuracy(DepProbMatrix model, ArrayList<WordInstance> depInsts, int[][] parses, HashMap<Pair<String, String>,Pair<Integer, Integer>> universalRule, double regulationValue) {
		int totalWords = 0;
		int numDirCorrect = 0;
...
//			}
			...
			parseSentence(sd, parse,  universalRule, regulationValue);
	 ...
	 * to get directed and undirected accuracy figures.
	 */
	public static double[] computeAccuracy(DepSentenceDist[] sentDists, int[][] parses, DepCorpus c, HashMap<Pair<String, String>,Pair<Integer, Integer>> universalRule, double regulationValue) {
		int totalWords = 0;
		int numDirCorrect = 0;
		int numUnDirCorrect = 0;
